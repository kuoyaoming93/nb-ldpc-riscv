...
Z = ['[C] T-MM LAYERED (' num2str(iter) ' iter), FLOAT, SF=1 '];
Y1 = zeros(1,length(EBNO));
for j=1:length(EBNO)
    for i=1:length(seed)
        for k = 1:length(Nerrors)
            archivo2 = ['../RESULTS_ETMM_q16_dc4_float/(' num2str(N) ',' num2str(K) ')_L' num2str(Nerrors(k)) '[' num2str(seed(i)) ']_N' num2str(N*p) '_K' num2str(K*p) '_EBN' num2str(EBNO(j),'%2.2f') '.txt'];
